/* ── APEIRON BUVETTE — PDF TICKET GENERATOR ─────────────────
   Requires jsPDF loaded in page before this script.
   Usage: PDF.generate(buvette, order)
─────────────────────────────────────────────────────────── */
const PDF = {
  generate(buvette, order) {
    const { jsPDF } = window.jspdf;
    const W = 80, margin = 5, cw = W - margin * 2;
    const doc = new jsPDF({ unit:'mm', format:[W, 10] }); // height auto-expanded
    let y = margin;
    ...
    const solidLine = (color=[100,100,110]) => {
